...
namespace TeekayUtils.DevConsole
{
    ...
    public sealed class DevConsole : PersistentSingleton<DevConsole>
    {
        ...
        /// <summary>
        /// Explicit init. Not required — any other static API will trigger lazy creation.
        /// Also the ONLY way to revive the console after a previous instance was destroyed
        /// mid-session: s_shuttingDown gates every other entry point (so scene-teardown
        /// callbacks can't resurrect the singleton by accident), and only an explicit
        /// Initialize clears it. The base isQuitting guard still blocks resurrection
        /// during real application shutdown.
        /// </summary>
        public static void Initialize()
        {
            if (!isQuitting) s_shuttingDown = false;
        ...
        protected override void Awake()
        {
            ...
            // Revive the static API after a previous instance was destroyed mid-session
            // (OnDestroy sets s_shuttingDown; without this reset a re-created console
            // silently ignores every Register/Execute call - caught by PlayMode tests).
