...
- **Every asset needs a committed `.meta`, ending with a trailing newline.** Unity's YAML parser rejects metas without the final `\n` and SILENTLY ignores the asset (no recompile, stale Test Runner, survives restarts). Never generate metas with `-NoNewline`; verify the last byte is LF and that GUIDs are unique repo-wide.
  - **A missing `.meta` hurts CONSUMERS far worse than DevProject.** Here the package is a `file:` dependency, so Unity just generates the meta. A consumer resolving the git URL gets an IMMUTABLE `Library/PackageCache` folder where Unity cannot write one — the script is never imported and its types are **silently absent from `TeekayUtils.dll`**. The consumer then gets `CS0246: type or namespace not found` for those types ONLY, while every other type in the package resolves, which reads like a using/namespace mistake in their own code rather than a missing file here. Measured 2026-07-30 after v3.3.0 shipped four meta-less scripts: `grep -c TypeName Library/ScriptAssemblies/TeekayUtils.dll` gave 0 for all four and 3 for their meta-bearing neighbours. **A release is not finished until every new file has a committed `.meta`** — open DevProject once to generate them, or copy an existing `.cs.meta` and swap in a fresh GUID.
